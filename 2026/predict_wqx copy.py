...
# 1) 参数区（你主要改这里）
# =========================
# ---- ROI：启动时交互框选，见下方"7) 框选ROI" ----
x0 = y0 = side = None

# ---- 去畸变 maps（必须与 ROI 一致生成）----
# 直接定义绝对路径字符串
MAPS_NPZ = r"G:\wqx\fisheye\zhiqiguanqujibian\undistort_maps_zhiqiguan_roi.npz"
# 验证文件是否存在
import os
...
def make_octagon_mask(side: int, cut_x: int, cut_y: int) -> np.ndarray:
    """对称八边形掩码：从正方形四角各切除一个三角形"""
...
# 7) ROI 与八边形参数（固定值）
# =========================
x0, y0, side = 480, 240, 582
cut_x, cut_y  = 112, 113

print(f"ROI: x0={x0}, y0={y0}, side={side}")
print(f"八边形: cut_x={cut_x}, cut_y={cut_y}")

octagon_mask = make_octagon_mask(side, cut_x, cut_y)
...
while True:
    ...
    if roi_raw.shape[0] != side or roi_raw.shape[1] != side:
        ...
        if key in [27, KEY_QUIT_Q]:
            break
        continue
    frame_roi_display = roi_raw.copy()  # 保留干净原图用于显示

    # ---- 去畸变 ----
    und = cv2.remap(roi_raw, map1, map2, cv2.INTER_LINEAR, borderMode=cv2.BORDER_CONSTANT)

    # ---- 深度推理（去畸变后全图）----
    depth_m = depth_anything.infer_image(und, INPUT_SIZE).astype(np.float32)
    ...
    mask_h, mask_w = octagon_mask.shape
    ...
    if len(contours) > 0:
        ...
        # --- 用轮廓做 mask 去取稳健深度 ---
        stone_mask = np.zeros(und.shape[:2], dtype=np.uint8)
        cv2.drawContours(stone_mask, [cnt], -1, 255, thickness=cv2.FILLED, lineType=cv2.LINE_AA)
        stone_mask = (stone_mask * octagon_mask).astype(np.uint8)
    ...
    # 显示窗口（全部为ROI方形裁剪）
    # =========================
    cv2.imshow("original_roi", frame_roi_display)
    cv2.imshow("annotated_roi", und)
