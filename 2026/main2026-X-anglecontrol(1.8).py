...
# 深度推理配置
DEPTH_ENCODER = 'vits'  # 模型编码器类型
DEPTH_INPUT_SIZE = 518  # 模型输入尺寸
DEPTH_GRAYSCALE = False  # 是否使用灰度深度图
# 圆形ROI参数（拉伸后画面）
DEPTH_CIRCLE_CENTER_X = 400
DEPTH_CIRCLE_CENTER_Y = 230
DEPTH_CIRCLE_RADIUS = 115
# 目标宽高比
DEPTH_TARGET_RATIO = 16/9
...
# ---------------- 深度推理相关函数 ----------------
def extract_circular_roi(image, center_x, center_y, radius):  # type: ignore
    """
    ...
    """
    # 1. 创建圆形掩码
    mask = np.zeros((image.shape[0], image.shape[1]), dtype=np.uint8)  # type: ignore
    cv2.circle(mask, (center_x, center_y), radius, 255, -1)  # type: ignore
    ...
    roi_image = np.copy(crop_image)  # type: ignore
    roi_image[crop_mask == 0] = 128  # 中性灰（模型对中性灰无响应）

    return roi_image, crop_mask

def resize_with_padding(image, target_ratio=16/9):  # type: ignore
    ...
    if orig_ratio > target_ratio:
        ...
        resized = cv2.resize(image, (new_width, new_height))  # type: ignore
        pad_width = (orig_width - new_width) // 2
        padded = cv2.copyMakeBorder(resized, 0, 0, pad_width, pad_width, cv2.BORDER_CONSTANT, value=[0, 0, 0])  # type: ignore
    else:
        new_width = orig_width
        new_height = int(new_width / target_ratio)
        resized = cv2.resize(image, (new_width, new_height))  # type: ignore
        pad_height = (orig_height - new_height) // 2
        padded = cv2.copyMakeBorder(resized, pad_height, pad_height, 0, 0, cv2.BORDER_CONSTANT, value=[0, 0, 0])  # type: ignore

    return padded

def depth_processing() -> None:
    """深度推理线程：持续运行深度推理并显示结果"""
    if not depth_available or cv2 is None or torch is None or np is None or matplotlib is None or DepthAnythingV2 is None:
        print("深度推理库不可用，跳过深度处理线程")
        return
    ...
    try:
        ...
        print(f"深度推理使用设备：{DEVICE}")

        # 加载模型
        depth_anything = DepthAnythingV2(**model_configs[DEPTH_ENCODER])  # type: ignore
        weight_path = f'checkpoints/depth_anything_v2_{DEPTH_ENCODER}.pth'
        if not os.path.exists(weight_path):
            print(f"错误：深度权重文件不存在 → {weight_path}")
            return
        depth_anything.load_state_dict(torch.load(weight_path, map_location=DEVICE))  # type: ignore
        depth_anything = depth_anything.to(DEVICE).eval()  # type: ignore

        # 打开摄像头
        cap = cv2.VideoCapture(0)  # type: ignore
        if not cap.isOpened():
            print("错误：深度推理无法打开摄像头！")
            return

        cmap = matplotlib.colormaps.get_cmap('Spectral_r')  # type: ignore
        margin_width = 0

        print("深度推理线程已启动，开始实时深度推理...")
        ...
        with torch.no_grad():  # type: ignore
            while True:
                ...
                # ========== 步骤1：拉伸画面 ==========
                stretched_frame = resize_with_padding(raw_frame, target_ratio=DEPTH_TARGET_RATIO)

                # ========== 步骤2：提取纯圆形有效像素 ==========
                roi_image, crop_mask = extract_circular_roi(
                    stretched_frame,
                    center_x=DEPTH_CIRCLE_CENTER_X,
                    center_y=DEPTH_CIRCLE_CENTER_Y,
                    radius=DEPTH_CIRCLE_RADIUS
                )

                # ========== 步骤3：仅对纯圆形区域推理 ==========
                depth = depth_anything.infer_image(roi_image, DEPTH_INPUT_SIZE)

                # ========== 步骤4：深度图后处理 ==========
                # 归一化
                depth = (depth - depth.min()) / (depth.max() - depth.min()) * 255.0
                depth = depth.astype(np.uint8)  # type: ignore

                # 仅保留圆形掩码内的深度值，其余设为0（黑色）
                depth_masked = np.zeros_like(depth)  # type: ignore
                depth_masked[crop_mask == 255] = depth[crop_mask == 255]  # 只保留圆形深度

                # 可视化
                if DEPTH_GRAYSCALE:
                    depth_vis = np.repeat(depth_masked[..., np.newaxis], 3, axis=-1)  # type: ignore
                else:
                    # 先生成彩色图，再掩码过滤
                    depth_color = (cmap(depth)[:, :, :3] * 255)[:, :, ::-1].astype(np.uint8)  # type: ignore
                    depth_vis = np.zeros_like(depth_color)  # type: ignore
                    depth_vis[crop_mask == 255] = depth_color[crop_mask == 255]

                # ========== 步骤5：显示 ==========
                # 统一缩放（保持尺寸一致）
                crop_size = max(roi_image.shape[0], roi_image.shape[1])
                roi_image_resized = cv2.resize(roi_image, (crop_size, crop_size))  # type: ignore
                depth_vis_resized = cv2.resize(depth_vis, (crop_size, crop_size))  # type: ignore

                # 拼接显示（纯圆形原始帧 + 纯圆形深度帧）
                split_region = np.ones((crop_size, margin_width, 3), dtype=np.uint8) * 255  # type: ignore
                combined_frame = cv2.hconcat([roi_image_resized, split_region, depth_vis_resized])  # type: ignore

                cv2.imshow('实时深度推理（圆形ROI）', combined_frame)  # type: ignore
                if cv2.waitKey(1) & 0xFF == ord('q'):  # type: ignore
                    break

        cap.release()  # type: ignore
        cv2.destroyWindow('实时深度推理（圆形ROI）')  # type: ignore
        print("深度推理线程退出成功！")

    except Exception as exc:
        print(f"深度推理线程异常：{exc}")
        try:
            cv2.destroyWindow('实时深度推理（圆形ROI）')  # type: ignore
        except:
            pass
...
class VoiceStatusWindow:
    ...
    def _run(self) -> None:
        ...
        try:
            ...
            # 启动Tkinter的事件循环（这会阻塞当前线程）
            # 注意：这是在单独的线程中，所以不会阻塞主程序
            print("语音控制UI窗口已创建，正在启动...")
            self._poll_queue()
            self.root.mainloop()
...
def main() -> None:
    ...
    depth_thread = None
    if DEPTH_SWITCH and depth_available:
        depth_thread = threading.Thread(
            target=depth_processing,
            name="depth_thread",
            daemon=ROBOT_SWITCH,
        )
        depth_thread.start()
        print(f"{get_time()}-depth_thread 已启动 (Depth_switch=True)")
    elif DEPTH_SWITCH and not depth_available:
        print(f"{get_time()}-Depth_switch=True但深度推理库不可用，跳过深度线程启动")
    else:
        print(f"{get_time()}-Depth_switch=False，跳过深度推理线程启动")
