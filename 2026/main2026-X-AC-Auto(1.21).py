...
def video_processing() -> None:
    ...
    try:
        from predict_2026 import UnetPackage  # type: ignore[import]
...
class VisionRobotStateMachine:
    ...
    def loop_vision_control(self) -> None:
        ...
        # 打印 pos_info 的具体内容（已移除打印），便于确认哪些 target 被设置
    ...
    def _apply_manual_motion(self, forward_speed: float, horizontal_speed: float, vertical_speed: float) -> None:
        ...
        try:
            ...
            with self.manual_input_lock:
                ...
                # # 调试输出：手动模式M2输入和计算
                # print(f"{get_time()}-手动模式M2: 输入vertical_speed={vertical_speed:.6f}, 灵敏度系数={self.m2_sensitivity_multiplier:.3f}, 计算后d2={d2:.6f}")
                if self.voice_window:
                    self.voice_window.push_log(f"手动模式M2输入: vertical_speed={vertical_speed:.6f}, d2={d2:.6f}")
                ...
                if abs(new_m2 - self.manual_targets['m2']) >= self.manual_min_delta_deg:
                    ...
                    if abs(new_m2 - filtered_prev) >= self.m2_deadband_deg:
                        ...
                        try:
                            ...
                            if self.voice_window:
                                self.voice_window.push_log(f"手动模式M2位置控制: 目标角度={new_m2:.3f}°")
    ...
    def _vision_position_mode(self, horizontal_input: float, vertical_input: float, current_m1: float, current_m2: float) -> Dict[str, float]:
        ...
        delta_h = self._vision_position_delta(horizontal_input, VISION_POSITION_GAIN_HORIZONTAL * self.sensitivity_multiplier)
        if delta_h != 0.0:
            # 强制最小移动步长：如果计算出的 delta 小于最小步长，则使用最小步长的方向性替代
            raw_target = self.vision_targets['m1'] + delta_h
            diff = raw_target - self.vision_targets['m1']
            if abs(diff) < getattr(self, "vision_m1_min_step", 1.0):
                diff = self.vision_m1_min_step * (1.0 if diff >= 0 else -1.0)
            target = self._clamp_angle(self.vision_targets['m1'] + diff, self.motors.m1_limit)
            if abs(target - self.vision_targets['m1']) >= 0.01:  # 进一步降低阈值，确保视觉控制能响应
                # 记录前一次已知位置（缓存）
                try:
                    _, prev_m1, _ = self.motors.get_cached_angles()
                except Exception:
                    prev_m1 = self.vision_targets.get('m1', 0.0)
                ...
                try:
                    ...
                    # 立即尝试读取并获取 M1 实际位置
                    try:
                        self.motors.m1.update_state()
                        pos_after = float(self.motors.m1.position)
                    except Exception:
                        pos_after = None

                    # 后备：当位置没有明显变化时使用速度 nudge 验证硬件响应
                    try:
                        moved_threshold = 0.05
                        prev_val = prev_m1 if prev_m1 is not None else 0.0
                        if pos_after is None or abs(pos_after - prev_val) < moved_threshold:
                            nudge_speed = 3.0 if (target - prev_val) >= 0 else -3.0
                            try:
                                self.motors.map_horizontal(nudge_speed)
                                time.sleep(0.12)
                                self.motors.map_horizontal(0.0)
                                # 再次读取位置
                                try:
                                    self.motors.m1.update_state()
                                    pos_after2 = float(self.motors.m1.position)
                                except Exception:
                                    pass
                            except Exception:
                                pass
                    ...
                    print(f"{get_time()}-ERROR: set_motor_position for M1 failed: {exc}")
        ...
        # 说明：
        #  - 加入换向检测和快速反转机制
        #  - 当检测到方向改变时，使用速度控制进行快速反转
        #  - 模拟手动模式下摇杆最大角度的响应速度
        # 设计思路：
        #  1) 检测方向改变（基于vertical_input的符号变化）
        #  2) 换向时立即使用最大速度反转一段时间
        #  3) 反转完成后恢复正常的位置控制

        # 初始化方向历史记录（用于检测换向）
        if not hasattr(self, 'last_vertical_direction'):
            self.last_vertical_direction = 0
        if not hasattr(self, 'direction_change_time'):
            self.direction_change_time = 0

        # 获取当前方向（1=向下，-1=向上，0=停止）
        current_vertical_direction = 1 if vertical_input > 0.5 else (-1 if vertical_input < -0.5 else 0)

        # 检测方向改变
        direction_changed = (current_vertical_direction != 0 and
                           self.last_vertical_direction != 0 and
                           current_vertical_direction != self.last_vertical_direction)

        # 更新方向历史
        self.last_vertical_direction = current_vertical_direction

        # 检查是否在换向冷却期内（避免连续触发）
        current_time = time.time()
        in_cooldown = (current_time - self.direction_change_time) < 0.5  # 0.5秒冷却

        # 换向处理：使用速度控制进行快速反转
        if direction_changed and not in_cooldown:
            self.direction_change_time = current_time
            # 使用最大速度进行反转（模拟摇杆最大角度）
            reverse_speed = TURN_COEFF if current_vertical_direction > 0 else -TURN_COEFF
            reverse_duration = 0.15  # 反转持续时间（秒）

            print(f"{get_time()}-检测到M2换向! 从{self.last_vertical_direction}到{current_vertical_direction}，使用速度{reverse_speed}反转{reverse_duration}s")
            if self.voice_window:
                self.voice_window.push_log(f"M2换向反转: 速度{reverse_speed}, 时长{reverse_duration}s")

            try:
                # 发送反转速度脉冲
                self.motors.map_vertical(reverse_speed)
                time.sleep(reverse_duration)
                self.motors.map_vertical(0.0)  # 停止

                # 重新同步位置
                try:
                    self.motors.m2.update_state()
                    current_pos = float(self.motors.m2.position)
                    self.vision_targets['m2'] = current_pos
                    self.vision_targets_filtered['m2'] = current_pos
                    print(f"{get_time()}-M2换向后位置: {current_pos:.3f}°")
                except Exception:
                    pass

            except Exception as exc:
                print(f"{get_time()}-M2换向反转失败: {exc}")

            # 标记已移动
            info["m2_target"] = self.vision_targets.get('m2', 0.0)
            moved = True
            ...
            # 获取当前实际电机位置作为基准
            try:
                _, _, current_actual_m2 = self.motors.get_cached_angles()
            except Exception:
                current_actual_m2 = 0.0

            # 将视觉输入映射为相对于当前位置的移动增量
            vision_to_angle_scale = 0.1  # 视觉输入10对应1°移动
            desired_move = vertical_input * vision_to_angle_scale

            # 计算目标角度：当前实际位置 + 期望移动
            target_m2 = self._clamp_angle(current_actual_m2 + desired_move, self.motors.m2_limit)

            # 调试输出
            print(f"{get_time()}-视觉模式M2: 输入vertical_input={vertical_input:.6f}, 当前实际位置={current_actual_m2:.3f}°, 期望移动={desired_move:.6f}°, 目标角度={target_m2:.3f}°")
            if self.voice_window:
                self.voice_window.push_log(f"视觉模式M2: 当前={current_actual_m2:.3f}°, 目标={target_m2:.3f}°")

            # 使用与手动模式相同的控制逻辑，但基于实际位置
            with self.manual_input_lock:  # 使用相同的锁
                # 计算与当前目标的差异（模拟d2的概念）
                d2 = target_m2 - self.vision_targets.get('m2', current_actual_m2)

                # 确保 vision_targets['m2'] 已初始化
                if abs(self.vision_targets.get('m2', 0.0)) < 1e-6:
                    self.vision_targets['m2'] = current_actual_m2
                    self.vision_targets_filtered['m2'] = current_actual_m2

                # 计算新的目标角度（基于实际位置的累积）
                new_m2 = self._clamp_angle(self.vision_targets['m2'] + d2, self.motors.m2_limit)

                # 只有当新目标与当前目标差异大于最小增量时才发送命令
                if abs(new_m2 - self.vision_targets['m2']) >= self.manual_min_delta_deg:
                    # 更新 raw 目标
                    self.vision_targets['m2'] = new_m2
                    filtered_prev = self.vision_targets_filtered.get('m2', new_m2)
                    ...
                    if abs(new_m2 - filtered_prev) >= self.m2_deadband_deg:
                        ...
                        # 构造并输出一条简洁的日志信息（与手动模式格式相同）
                        debug_msg = (
                            f"M2 vision control -> new:{new_m2:.3f} filtered_prev:{filtered_prev:.3f} "
                            f"deadband:{self.m2_deadband_deg} cached_m2:{cached_m2 if cached_m2 is None else f'{cached_m2:.3f}'}"
                        )
                        print(f"{get_time()}-{debug_msg}")
                        if self.voice_window:
                        ...
                        # 更新 filtered 目标并尝试发送位置命令（位置闭环）
                        try:
                            self.vision_targets_filtered['m2'] = new_m2
                            print(f"{get_time()}-视觉模式M2位置控制: 发送位置命令 -> 目标角度={new_m2:.3f}°, max_speed={TURN_COEFF}")
                            if self.voice_window:
                        ...
                        try:
                            self.motors.m2.update_state()
                            cur_pos = float(self.motors.m2.position)
                            print(f"{get_time()}-M2 vision post-set position: {cur_pos:.3f}°")
                        except Exception as exc:
                            print(f"{get_time()}-M2 vision position read error: {exc}")

                        # 如果位置没有明显变化，尝试 fallback（与手动模式相同）
                        if cur_pos is not None:
                            try:
                                prev_val = cached_m2 if cached_m2 is not None else filtered_prev
                                if abs(cur_pos - prev_val) < self.m2_deadband_deg:
                                    print(f"{get_time()}-M2 vision fallback: speed {2.0 if (new_m2 - prev_val) >= 0 else -2.0} for {0.12}s")
                                    if self.voice_window:
                                        self.voice_window.push_log(f"M2 vision fallback: speed {2.0 if (new_m2 - prev_val) >= 0 else -2.0}")

                                    # 发送速度脉冲
                                    nudge_speed = 2.0 if (new_m2 - prev_val) >= 0 else -2.0
                                    ...
                                    try:
                                        self.motors.m2.update_state()
                                        final_pos = float(self.motors.m2.position)
                                        print(f"{get_time()}-M2 vision fallback post position: {final_pos:.3f}°")
                                        if self.voice_window:
                                            self.voice_window.push_log(f"M2 vision fallback post: {final_pos:.3f}°")

                                        # 更新目标位置为实际位置，确保下次控制的连续性
                                        if abs(final_pos - prev_val) >= self.m2_deadband_deg:
                                            self.vision_targets['m2'] = final_pos
                                            self.vision_targets_filtered['m2'] = final_pos
                                    except Exception as exc:
                                        print(f"{get_time()}-M2 vision fallback position read error: {exc}")
                            except Exception as exc:
                                print(f"{get_time()}-M2 vision fallback error: {exc}")

                        info["m2_target"] = new_m2
                        moved = True
