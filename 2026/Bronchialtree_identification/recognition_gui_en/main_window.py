...
class RecognitionMainWindow(QMainWindow):
    def __init__(self, robot_runtime: RobotRuntime) -> None:
        ...
        self.setWindowTitle("Bronchoscopy Position Recognition and Navigation System")
    ...
    def _build_ui(self) -> None:
        ...
        title = QLabel("Bronchoscopy Position Recognition and Navigation System")
        title.setObjectName("title")
        subtitle = QLabel("REAL-TIME ANATOMICAL LOCALIZATION & ROBOT NAVIGATION")
        ...
        self.position_badge = self._badge("Position: Trachea [TR]", "activeBadge")
        self.motion_badge = self._badge("Motion: S", "neutralBadge")
        self.controller_badge = self._badge("Controller: Waiting", "neutralBadge")
        self.camera_badge = self._badge("Bronchoscope: Waiting", "neutralBadge")
        for badge in (
            self.position_badge,
            self.motion_badge,
        ...
        video_title = QLabel("LIVE BRONCHOSCOPE VIEW")
        video_title.setObjectName("panelTitle")
        self.video_detail = QLabel("Waiting for video worker")
        ...
        self.video_label = QLabel("Loading bronchoscope video...")
        ...
        self.detection_switch = QCheckBox("Object Detection")
        ...
        self.manual_button = self._command_button("Manual Mode", "manual", "secondaryButton")
        self.stop_button = self._command_button("Stop", "stop", "stopButton")
        self.zero_button = self._command_button("Return to Zero", "zero", "secondaryButton")
        self.set_zero_button = QPushButton("Set Current as Zero")
    ...
    def _build_menus(self) -> None:
        inspection_menu = self.menuBar().addMenu("Inspection")
        start_inspection = QAction("Start Autonomous Inspection (Latest RMB Path)", self)
        start_inspection.triggered.connect(self.robot_runtime.start_inspection)
        stop_inspection = QAction("Stop Inspection and Return to Idle", self)
        ...
        navigation_menu = self.menuBar().addMenu("Auto Navigation")
        ...
        pause_navigation = QAction("Pause / Resume Navigation", self)
        pause_navigation.triggered.connect(self.robot_runtime.pause_navigation)
        stop_navigation = QAction("Stop Navigation", self)
    ...
    def _refresh_status(self) -> None:
        ...
        if not detection_requested:
            self.position_badge.setText("Position: Detection Off")
            ...
            if self._display_gate.armed:
                ...
                self.position_badge.setText(
                    f"Position: {current_name} [{detection.confirmed_position}]"
                )
                self.map_widget.set_detection_state(
                    detection.confirmed_position,
                ...
                self.position_badge.setText("Position: Waiting for Recognition")
                self.map_widget.set_detection_state(
                    None,
                    None,
                    "UNCERTAIN",
                    "Detection is on; waiting for a validated position",
                )

        motion_codes = {"forward": "F", "backward": "B", "idle": "S"}
        mode_text = robot.state if robot.ready else ("Connection Failed" if robot.error else "Connecting")
        self.motion_badge.setText(
            f"Mode: {mode_text} | Motion: {motion_codes.get(motor.motion_mode, 'S')}"
        )
        ...
        self.controller_badge.setText(
            "Controller: Connected" if robot.controller_connected else "Controller: Disconnected"
        )
        ...
        self.camera_badge.setText(
            "Bronchoscope: Connected" if video.camera_connected else "Bronchoscope: Disconnected"
        )
    ...
    def _toggle_detection(self, enabled: bool) -> None:
        ...
        GUI_MESSAGE_BUS.publish("Object detection enabled" if enabled else "Object detection disabled")
    ...
    def _confirm_set_zero(self) -> None:
        answer = QMessageBox.question(
            ...
            "Set the current three-axis motor positions as zero?",
    ...
    def _confirm_poweroff(self) -> None:
        answer = QMessageBox.warning(
            self,
            "Confirm Power Off",
            "The robot will enter a powered-off maintenance state. Continue?",
            QMessageBox.Yes | QMessageBox.No,
            QMessageBox.No,
        )
        if answer == QMessageBox.Yes:
    ...
    def closeEvent(self, event: QCloseEvent) -> None:  # noqa: N802 - Qt API
        ...
        answer = QMessageBox.question(
            self,
            "Safe Exit",
            "Stop recording, close the video stream, and safely shut down the robot?",
            QMessageBox.Yes | QMessageBox.No,
            QMessageBox.No,
        )
        if answer != QMessageBox.Yes:
