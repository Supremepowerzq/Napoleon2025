...
# ★改成你真实的 Depth-Anything-V2-main 路径
REPO_ROOT = Path(r"E:\wqx\Depth-Anything-V2-main")
...
LOAD_FROM = Path(r"E:\wqx\Depth-Anything-V2-main\checkpoints\depth_anything_v2_metric_hypersim_vitl.pth")
...
class UnetPackage:
    ...
    def crop_to_circle(self, image, fps=None, show_crosshair=True, show_circles=True, red_circles=None):
        """
        将图像裁剪为圆形区域，黑边尽量少，圆形居中显示（与 predict_2026_wqx 一致）
        ...
        try:
            ...
            if show_circles:
                circle_centers = [(cw - 40, ch - 20), (cw - 20, ch - 40), (cw - 60, ch - 40), (cw - 40, ch - 60)]
    ...
    def video(self):
        ...
        while True:
            ...
            r_frame = side_frame // 2
            cv2.circle(circle_mask_frame, (cx_frame, cy_frame), r_frame, 255, -1)

            # ===== 深度推理：只在frame_cropped_raw上做一次推理，复用于显示和深度计算 =====
            depth_result_m = None
            depth_display = None
            if self.enable_depth:
                try:
                    ...
                    # 单次深度推理（米）
                    depth_result_m = depth_anything.infer_image(frame_for_depth, INPUT_SIZE).astype(np.float32)
                    ...
                    # 深度模型输出是方形的，需要先resize回原始帧尺寸再处理
                    if depth_uint8.shape[:2] != (h_frame, w_frame):
                        depth_resized = cv2.resize(depth_uint8, (w_frame, h_frame), interpolation=cv2.INTER_LINEAR)
                    else:
                        depth_resized = depth_uint8

                    # 先resize再应用颜色映射，确保输出与原始帧尺寸一致
            ...
            roi_image[circle_mask_frame == 0] = 128
            ...
            if  len(contours2) > 0:
                ...
                # ===== 使用之前单次深度推理的结果计算目标深度 =====
                # 复用 depth_result_m（已在上面计算），无需再次推理
                depth_m = depth_result_m
            ...
            # ===== 拼接三个窗口（与 predict_2026_wqx 一致：同一尺寸 h,w，深度图圆外黑色掩膜）=====
            try:
                h, w = circular.shape[:2]

                if depth_display is None:
                    depth_display_circular = np.zeros((h, w, 3), dtype=np.uint8)
                else:
                    ...
                    # 与 wqx 一致：圆心与半径可调，圆外黑色掩膜
                    depth_center_x = w_d // 2 - 25
                    depth_center_y = h_d // 2
                    depth_radius = min(h_d, w_d) // 2 - 65
                    ...
                    if depth_display_circular.ndim == 2:
                        depth_display_circular = cv2.cvtColor(depth_display_circular, cv2.COLOR_GRAY2BGR)
                    depth_display_circular[mask_d == 0, :] = 0

                combined_width = w * 3
                combined_height = h
                combined_image = np.zeros((combined_height, combined_width, 3), dtype=np.uint8)
                combined_image[:, 0:w] = circular
                combined_image[:, w:2*w] = circular_frame
                combined_image[:, 2*w:3*w] = depth_display_circular
