...
try:
    ...
    from BC.data_collector import BronchusDataCollector, update_visual_features as _bc_update_vis
...
def video_processing() -> None:
    ...
    try:
        from predict_2026_528_zck import UnetPackage  # type: ignore[import]
...
class VoiceStatusWindow:
    ...
    def _run(self) -> None:
        ...
        try:
            ...
            # ── BC 路径选择器（仅在 BC 模块可用时显示）─────────────────
            if _BC_AVAILABLE and _BC_PATHS:
                path_sel_frame = tk.Frame(action_frame, bg="white")
                path_sel_frame.pack(fill=tk.X, pady=(0, 6))
                tk.Label(path_sel_frame, text="BC录制路径:", font=("Microsoft YaHei", 10),
                         bg="white", fg="#555").pack(side=tk.LEFT, padx=(0, 6))
                # 构建选项列表：["EXP - 自主探索", "TR - 气管", ...]
                path_options = [
                    f"{v[0]} - {v[1]}" for v in _BC_PATHS.values()
                ]
                self.bc_path_var = tk.StringVar(value=path_options[2] if len(path_options) > 2 else path_options[0])
                path_menu = tk.OptionMenu(path_sel_frame, self.bc_path_var, *path_options)
                path_menu.config(width=20, font=("Microsoft YaHei", 9))
                path_menu.pack(side=tk.LEFT)
                self.bc_path_label = tk.Label(path_sel_frame, text="",
                                              font=("Microsoft YaHei", 9), bg="white", fg="#888")
                self.bc_path_label.pack(side=tk.LEFT, padx=8)
            # ──────────────────────────────────────────────────────────
...
class MotorGroup2025:
    def __init__(self, ser: serial.Serial) -> None:
        ...
        self.m2_limit = (-500.0, 500.0)  # ★ [修改3] 电机2限位：±180度（原±15°，加装10倍减速器后改大）
...
class VisionRobotStateMachine:
    ...
    states = ('Idle', 'ManualControl', 'VisionControl', 'AIAuto', 'PowerOff')
    ...
    def __init__(self) -> None:
        ...
        self._AI_MAX_STEPS      = 1800           # 最大推理步数（30s × 60Hz）
    ...
    def _transition_to_ai_auto(self, goal_id: Optional[int] = None) -> None:
    ...
    def _setup_action_recorder(self) -> None:
        ...
        try:
            ...
            if _BC_AVAILABLE and BronchusDataCollector is not None:
                try:
                    import os as _os2
                    bc_demo_dir = _os2.path.join(_os2.path.dirname(__file__), "BC", "expert_demos")
                    self.bc_collector = BronchusDataCollector(
                        motor_group=self.motors,
                        save_dir=bc_demo_dir,
                        record_hz=20.0,
                        path_label=2,   # 默认 LMB，会在录制开始时按 UI 选择更新
                    )
    ...
    def _on_action_record(self, start: bool) -> None:
        """
        录制回调：
          - ActionRecorder 录制已关闭（不再写 recorded_actions/）
          - BronchusDataCollector 负责完整的 BC 训练数据采集
          - ActionPlayer 回放功能保持不变（读取已有的 recorded_actions/*.json）
        """
        if start:
            # ── 读取 UI 路径选择 ──────────────────────────────────
            path_label = 2  # 默认 LMB
            ...
            if self.bc_collector is not None:
                ...
                if self.bc_collector.start(session, path_label):
                    ...
                    if self.voice_window:
                        self.voice_window.push_log("[BC] 启动失败：可能已在录制中")
            else:
                if self.voice_window:
            ...
            if self.bc_collector is not None and self.bc_collector.is_recording:
                ...
                self.voice_window.push_log("[BC] 当前未在录制")
    ...
    def _vision_position_mode(self, forward_input: float, horizontal_input: float, vertical_input: float) -> Dict[str, float]:
        ...
        try:
            ...
            # M2 垂直轴：视觉模式专用缩放（vy_max=30 → 视觉速度最大30，对应M2约60%满速）
            # 原 m2_sensitivity_multiplier*60 公式针对手动滑块设计，视觉vy量级不同，不复用
            try:
                VISION_M2_SPD_SCALE = 8.0    # vy_max=8 → m2_spd最大64 deg/s，安全范围内可按需调整
                m2_spd = max(-M2_SPEED_COEFF, min(M2_SPEED_COEFF, vertical_input * VISION_M2_SPD_SCALE))
                if abs(m2_spd) > 5.0:        # 死区：避免微小vy噪声持续抖动
