    ...
    def video(self):
        ...
        while True:
            ...
            try:
                ...
                cv2.imshow('Original', circular)
            except Exception:
                cv2.imshow('Original', frame_cropped_raw)
            
            # ===== 第三步：对裁剪后的原始帧进行unet目标检测 =====
            frame = cv2.cvtColor(frame_cropped_raw, cv2.COLOR_BGR2RGB)
            ...
            if  len(contours2) > 0:
                ...
                # 重新定义circle_centers和circle_radius用于着色反馈（这些将被绘制在处理后的图像中）
