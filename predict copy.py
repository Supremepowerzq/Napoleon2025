...
class UnetPackage:
    ...
    def video(self):
        ...
        if self.video_save_path != "":
            # 直接用数字常量代替fourcc，防止属性不存在报错
            fourcc = 1145656920  # 'XVID'的fourcc编码
    ...
    def fps(self):
        ...
        tact_time = None
        if tact_time is not None and tact_time != 0:
            print(str(tact_time) + ' seconds, ' + str(1/tact_time) + 'FPS, @batch_size 1')
        else:
            print('tact_time未定义，无法计算FPS')
        t2 = time.time()
        if t2 - t1 != 0:
            print('FPS:', 1 / (t2 - t1))
        else:
            print('t2-t1为0，无法计算FPS')

    def directory(self):
        # 修正：如无detect_directory方法则注释掉
    ...
    def run(self):
        if self.mode == "predict":
            # 需传入image_path参数
            self.predict(self.fps_image_path)
