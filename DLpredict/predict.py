    ...
    def video(self):
        ...
        while True:
            ...
            if len(contours1) > 0:
                ...
                # thickness: 圆的厚度。如果为负值，则填充整个圆。
                # lineType: 线条类型。
                # shift: 坐标点的小数位数。

                result = cv2.drawContours(frame, contours1[max_num0], -1, (255, 160, 0), 2, cv2.LINE_AA)
            ...
            if len(contours2) > 0:
                ...
                if M['m00'] == 0:
                    ...
                    cx = int(M['m10']/M['m00'])
                    cy = int(M['m01']/M['m00'])
                # frame = cv2.circle(frame, (cx, cy), 3, (0, 0, 255), -1)#结石中心红点
                frame = cv2.circle(frame, (cx, cy), 8, (0, 0, 255), -1,cv2.LINE_AA)#结石中心红圆
                 #cv2.circle(img, center, radius, color, thickness=None, lineType=None, shift=None)
                # img: 输入图像。
                # center: 圆心坐标，格式为 (x, y)。
                # radius: 圆的半径。
            ...
            # 绘制水平线
            cv2.line(frame, (center_x - cross_length, center_y), (center_x + cross_length, center_y), (0, 0, 255), 2)

            # 绘制垂直线
            cv2.line(frame, (center_x, center_y - cross_length), (center_x, center_y + cross_length), (0, 0, 255), 2)
            ...
            cv2.drawContours(overlay, contours2, max_num, (0, 255, 0), -1)
            alpha = 0.3
